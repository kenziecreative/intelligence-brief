...
- **Register** joins every entry's applicable set. **Continuity** applies only to resume
  ...
  elsewhere.
- **`expected_no_advance: true` scenarios: Brief Coherence is n/a** when the run captured
  no stage result. Its anchors grade a produced section ("section has framework + result
  + takeaway"), and its 0 anchor is "section empty" — but these scenarios exist to test
  that the plugin *refuses* to capture a result the user never agreed to, so the section
  is empty because the run was correct. Scoring it would fail the plugin for the exact
  behavior under test, and under the 1.3.0 floor a 0 fails the scenario outright. This
  mirrors the carve-out `brief_section_filled` already has on the gate side ("mid-stage
  section writes are legitimate"); the judged dimension was left out when that was added.
