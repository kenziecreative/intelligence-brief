...
- **`dev/STATE.md` is the stream index** — which plugin streams exist, where each one's state lives, and the branching rule. It carries no stream detail and a checkpoint rarely edits it.
- **`dev/<plugin>/STATE.md` is that plugin's work snapshot** — where it stands, what's in flight, the next-steps queue, open decisions, session-only knowledge. Read the index, then the one for the plugin you're picking up. Both are written by the `/checkpoint` skill (`.claude/skills/checkpoint/`), which any session should run before clearing context so nothing lives only in conversation. A snapshot, not a log — overwrite it; git history holds old versions.
- **One plugin, one branch, branched from current `main`.** Work on plugin `X` touches `X/`, `dev/X/`, `eval/targets/X/`, and its own rows in the shared registries (`.claude-plugin/marketplace.json`, root `README.md`, this file). Those registries are row-per-plugin, so two streams editing different rows merge cleanly — verified across six concurrent branches. The genuinely shared files are `eval/lib/`, `dev/scripts/` and `.claude/skills/`; keep changes there small and merge them promptly instead of parking them on a long-lived plugin branch. Full rule and current stream status: `dev/STATE.md`.
